...
type UserRole struct {
	...
	UserId     uint      `json:"user_id" xorm:"notnull unique(ura) comment('用户ID')"` // 用户ID
	RoleId     uint      `json:"role_id" xorm:"notnull unique(ura) comment('角色ID')"` //  角色ID
	AppId      uint      `json:"app_id" xorm:"notnull unique(ura) comment('应用ID')"`  // 应用ID
