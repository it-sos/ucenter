...
type UserPermission struct {
	...
	UserId     uint      `json:"user_id" xorm:"notnull unique 'uk_userid_appid_routeid' comment('用户ID')"`  // 用户ID
	AppId      uint      `json:"app_id" xorm:"notnull unique 'uk_userid_appid_routeid' comment('应用ID')"`   // 应用ID
	RouteId    uint      `json:"route_id" xorm:"notnull unique 'uk_userid_appid_routeid' comment('路由ID')"` // 路由ID
